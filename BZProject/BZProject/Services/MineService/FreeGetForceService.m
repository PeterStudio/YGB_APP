...
@implementation FreeGetForceService
{
...
- (void)freeGetForce_Request_HTTP_MemberID:(NSString *)memberId
    ...
    // 发送POST请求
    [[AFHTTPRequestOperationManager manager] POST:HTTP_FreeGetForce_URL parameters:params success:^(AFHTTPRequestOperation *operation, id responseObject) {
        /*******成功返回处理逻辑**********/
        // 如果接口返回数据不加密，responseObject为NSDictionary类型，直接通过JSONModel方法反射出实体类
        FreeGetForceModel * demoModel = [[FreeGetForceModel alloc] initWithDictionary:responseObject error:nil];
        // 调用success块
        if (success) {
            success(demoModel);
        }
